...
static void atomic_bitwise(void)
{
        ...
        pr_info("Bits 0: " BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY(word));

        set_bit(3, &word);
        set_bit(5, &word);
        pr_info("Bits 1: " BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY(word));

        clear_bit(5, &word);
        pr_info("Bits 2: " BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY(word));

        change_bit(3, &word);
        pr_info("Bits 3: " BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY(word));

        if (test_and_set_bit(3, &word))
                pr_info("could not set 3rd bit\n");
        pr_info("Bits 4: " BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY(word));
