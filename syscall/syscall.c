 ...
 *
 * Disables page protection at a processor level by chaning the 16th bit
...
/* The in-kernel calls to the ksys_close() syscall were remove in v5.11+ */
...
/* The address of the sys_call_table, which can be obtained with lookup up
 * "/boot/System.map" or "/proc/kallsyms." When the kernel version is v5.7+,
 * without CONFIG_KPROBES, you can input the parameter or the module will
 * look up all the memory
 */
 ...
 * 100% safe, because if another module replaced sys_openat before us, then
 * when we are inserted, we will call the function in that module - and it
 * might be removed before we are
 *
 * Another reason for this is that we can not get sys_openat. It is a
 * static variable, so it is not exported
 */
 ...
 * In practice, the system calls almost never change (it would wreck havoc and
...
static unsigned long **acquire_sys_call_table(void)
{
        ...
        if (sym == 0) {
                pr_info("For Linux v5.7+, Kprobes is the preferable way to get symbol.\n");
                pr_info("If Kprobes is absent, you have to specific the address of sys_call_table symbol\n");
                pr_info("by /boot/System.map or /proc/kallsyms, which contains all the symbol addresses, into sym parameter.\n");
                return NULL;
        }
        sprint_symbol(symbo, sym);
...
static void __exit syscall_end(void)
{
        ...
        if (sys_call_table[__NR_openat] != (unsigned long *)our_sys_openat) {
                pr_info("Somebody else also played with the open system call\n");
                pr_info("The system may be left in an unstable state\n");
        }
